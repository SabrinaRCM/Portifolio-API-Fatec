...
* Principais Bibliotecas Python:  
    ...
    **Pandas / Matplotlib** - ferramentas para gerar e plotar gráficos;  
...
* Python - Eu havia tido contato com python uma única vez anterior ao projeto, em um evento. Neste projeto tive a oportunidade de compreender melhor como criar e executar uma aplicação em python com autonomia incluindo o uso das bibliotecas Django, Pandas e Matplotlib. 
...
* Comunicação - Nesse semestre tivemos que iniciar o ensino emergência a distância por conta da pandemia, portanto foi essencial manter uma comunicação mais eficaz por conta de não ter como se encontrar pessoalmente como anteriormente na faculdade.
...
* Thymeleaf - Utilizei pela primeira vez para facilitar o desenvolvimento da aplicação Java utilizando SpringBoot.
...
Para que essa lei seja cumprida, a empresa deve adequar seu banco de dados para que dados excluídos mediante solicitação do titular não sejam restaurados no backup ou outra forma.<br> 
...
```
#### Hard Skills Efetivamente Desenvolvidas
* Criptografia - Neste projeto foi utilizado o algoritmo de criptografia AES (Advanced Encryption Standard) de 256 bits. O algoritmo AES é uma cifra de bloco simétrico, ou seja, utiliza a mesma chave para cifrar e decifrar os dados e cada chave é única, caso seja perdida não será possível recuperar as informações cifradas. Obtive conhecimento sobre criptografia simétrica e assimétrica pois pesquisei e apliquei outros algoritmos antes de escolher o AES. A partir disso pude aprender mais sobre criptografia, como funciona, onde e como aplicar. 
* Object-Relational Mapping (ORM) - É um método de mapeamento objeto-relacional que permite fazer uma relação entre bancos de dados relacionais e linguagens orientadas a objetos. A implementação da biblioteca Peewee no projeto possibilitou compreender melhor a estrutura e importância da técnica.
